...
同步设置保存 `models[]` 和 `activeModelId`。每个模型包含稳定 ID、显示名称、Base URL、Model 参数、上下文窗口、输出上限、`supportsImages` 与 `pageRequestLimitPerMinute`。本地存储使用 `modelApiKeys[modelId]` 保存独立密钥。Chat 只读取脱敏模型列表，并通过 `SET_ACTIVE_MODEL` 修改当前 ID；background 在每次请求开始时重新解析当前模型和对应密钥。
...
| `storage.session` | 按列分析运行期缓存镜像 | 当前浏览器会话，默认 TTL 2 小时 |
...
整表分析技能支持 1–5 个数据源，可以来自同一页面的不同表格，也可以来自多个业务页面；按列分析当前仍是单数据源模型。配置保存在 `chrome.storage.local`；每个数据源独立保存稳定 ID、顶层页面、目标 frame、CSS selector、候选序号、字段指纹和绑定时的显示名称。旧版单数据源技能在读取时兼容为 `sources[0]`。
...
Chat 的“全部技能”区域继续按全局统一顺序编号。当前页面技能卡片使用显式“删除”按钮；“其他页面技能”区域只保留页面跳转和编号摘要，不再在摘要内提供单技能 `×` 删除入口，避免把导航与破坏性操作混在同一行内。

点击“其他页面技能”中的页面条目时，不再只把分组里的第一个数据源传给 background；前端会把该页面关联的候选数据源列表一并传递，background 在目标 tab 中依次尝试聚焦，直到命中第一个仍然可定位的数据源。因此同页存在多个技能、且首个技能已失效或已变化时，仍能滚动并高亮到后续可用技能对应的表格。

“其他页面技能”摘要区的展开/收起交互采用底边居中的控制柄。折叠态会保留至少两条技能摘要可见，控制柄本身下压到摘要区底边以下，并同步给下方“当前页面技能”标题增加上边距，避免控制柄覆盖两块内容的交界处。

“全部技能”顶部的导入 / 导出 / 删除全部技能属于独立的技能管理流：导出直接读取当前技能集合与页面显示名称并生成 JSON 文件；导入会先在前端执行 JSON 解析、大小限制、重复指纹过滤和数据源合法性校验，再把预览结果交由用户确认，最终由 background 在最新技能集合上串行合并写入。删除全部技能需要二次确认；当前页面技能的单条删除则直接通过显式“删除”按钮触发，并在成功后刷新目录、当前页卡片和表格上方技能条。

若扩展刚完成热更新、当前 content script 上下文已失效，技能导出与技能列表同步广播不会继续抛出原始 `Extension context invalidated`。内容脚本会把这类异常统一转成“扩展已更新，请刷新当前页面后重试”，避免控制台持续刷同步错误。
...
测试模式以一次全屏会话为缓存边界：只有所有数据源均已完整载入，且 `completeForRequest !== false` 时才允许复用；只要任一数据源因为用户停止、翻页失败、翻页状态不确定等原因留下不完整缓存，再次点击“开始测试 / 再次测试”都会重新采集，而不是要求退出工作台后重进。执行模式每次点击“执行技能”都会采集最新数据，“重新分析”只复用本次执行已经完整载入的数据。
...
自动执行只在技能配置 `trigger.autoRunEnabled = true` 时开启。页面刷新、列表变化或表格 DOM 被替换后，运行时会重新判断是否需要恢复结果列或重新请求；手动“更新”则带 `bypassPageGuard`、`ignoreCache` 和 `ignoreRecentResult`，用于在页面内强制重跑当前页。
