...
当前单元测试覆盖分组与时间编号、固定表头关联、adapter/rowKey、上下文隔离、token 预算、SSE chunk 边界、onboarding、Markdown/CSV 导出、模型新增草稿、技能导入去重、虚拟滚动规则、采集完成语义、技能 mutation/revision、旧版与新版 frame/表格定位，以及单表/多表请求分区、空单元格对齐、预算公平分配和数据完整性判断。静态回归负责全部源文件语法、Manifest 入口、调试日志、demo HTML 结构和关键导出检查。真实 Chrome E2E 还覆盖技能新建、修改、跨页多数据源持久化、测试全部载入、执行全部载入及每个数据源条数展示。
