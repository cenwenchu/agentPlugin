...
数据源读取采用可观察、可停止的分页采集会话。第一版复用表格模块的下一页识别和数据稳定判断，逐页读取并按整行指纹去重，默认限制为 10 页、1000 行。每页采集完成及翻页阶段会将进度从数据所在 frame 转发到顶层执行页；用户停止后保留已采集数据并继续分析。若分页器包含可识别的当前数字页码，采集结束后尝试恢复初始页。滚动加载、虚拟列表全量遍历及分页与滚动混合采集不属于本版范围。
