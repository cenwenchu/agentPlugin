...
当前单元测试覆盖分组与时间编号、固定表头关联、adapter/rowKey、上下文隔离、token 预算、SSE chunk 边界、onboarding 以及 Markdown/CSV 导出。静态回归负责语法、调试日志、demo HTML 结构和关键导出检查。真实 Chrome E2E 覆盖启动器关闭/恢复、表格能力开关、固定表头、同页相似表格隔离、底部汇总行排除、同源 iframe 注入、虚拟节点复用、分页式 DOM 替换以及刷新后内存清空。

## 10. 技能数据源（第一轮）

第一轮只实现单个数据源绑定，不调用模型。技能配置保存在 `chrome.storage.local`，以顶层页面的 `origin + pathname` 作为默认挂接范围。数据源同时记录目标 frame URL、CSS selector、同 frame 内部候选序号与数据源字段指纹。
...
技能配置分析方法后可进入独立的全屏测试模式，但仍复用 Chat 的模型配置、流式请求和 Markdown 渲染。第一版只读取数据源当前已经渲染且可从 DOM 获取的数据，不自动滚动或翻页；界面展示可读取总行数和前 5 行样例，最多向模型提交前 200 行。每次测试都会重新定位并读取数据源。模型按“结论、分析方法建议、分析过程”输出，其中建议优先检查输入条件与输出格式，并在必要时补充数据范围、字段口径及空值/异常值处理；流式结果自动滚动到最新内容。用户可以修改方法再次测试，满意后保存到技能。
