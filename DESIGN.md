...
当前单元测试覆盖分组与时间编号、固定表头关联、adapter/rowKey、上下文隔离、token 预算、SSE chunk 边界、onboarding、Markdown/CSV 导出、模型新增草稿、技能导入去重、虚拟滚动规则，以及单表/多表请求分区、空单元格对齐、预算公平分配和数据完整性判断。静态回归负责全部源文件语法、Manifest 入口、调试日志、demo HTML 结构和关键导出检查。真实 Chrome E2E 还覆盖技能新建、修改、跨页多数据源持久化、测试全部载入、执行全部载入及每个数据源条数展示。
...
选择数据源时 background 将选择命令广播到已打开业务页面的所有 frame，用户可切换页面点击目标表格，结果再返回原技能编辑页。页面刷新后严格按照“顶层页面 + frame URL + selector（其次为同 frame 保存序号）”定位，字段相似度只用于判断已定位表格是否发生变化，绝不用于在其他相似表格之间重新选择；相似度达到 80% 显示“可用”，能够定位但字段变化较大显示“数据源已变化”，无法定位显示“数据源失效”。旧版迁移生成的数据源强制继承技能原始页面，不能根据 frame 地址扩散页面归属。

技能配置分析方法后可进入全屏测试模式，并复用 Chat 的模型配置、流式请求和 Markdown 渲染。测试与执行共用 `skill-request-model.js`：客户分析方法保持原文，每个数据源按配置顺序生成独立标题、来源、字段和 Markdown 表格，不增加结论、列表或分析过程等输出要求。字符预算按数据源平均分配，单个大表截断时会标注实际提交行数，不能让靠后的数据源整体消失。
...
数据源读取采用可观察、可停止的分页采集会话。采集器逐页识别 Ant Design、Arco、ArtTable、通用虚拟滚动容器以及 iframe 文档级滚动，按约 75% 可视高度推进，等待渲染变化后按整行指纹去重。默认限制为 10 页、1000 行。进度从数据所在 frame 转发到顶层执行页；用户停止后结束本次完整数据装配，不提交缺少后续数据源的请求。每页结束恢复滚动顶部，整个任务结束后恢复业务分页第一页。当前仍不支持 canvas 表格；完全相同的重复业务行也可能因整行指纹去重而合并。

测试和执行都在用户点击开始后逐个预检数据源分页器。检测到多页时要求输入 1–10 或“全部”，默认值为 1，“全部”仍受 10 页上限约束。目标应用内页面已经关闭时，后台不会用当前页面的相似表格兜底，而会打开保存的页面地址；只有确认页面身份、结构和数据均正确后才写入会话。扩展自动创建的浏览器 Tab 必须等采集结果写入测试或执行状态后才能关闭。

多数据源采用严格完整性策略：全部数据源依次采集成功后才组装并提交模型。任意数据源未完成、页面无法打开、结构更新被拒绝或采集被终止时，本次不会把不完整数据提交给模型，界面会指出未完成的数据源。
...
第一版限制为每次最多 5 个临时文件、单文件 10MB、单工作表 10,000 行和 500 列；第一行作为表头，空表头生成“列N”。不支持旧二进制 `.xls`、密码保护工作簿、宏和外部链接刷新；公式使用文件中已有的缓存结果。
