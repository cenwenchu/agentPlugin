...
- 左侧“技能”支持创建技能并绑定当前已打开页面中的一个数据源；保存后会持久化页面、frame、数据源定位和数据源字段指纹
- 技能使用单个自然语言输入框描述分析需求，不要求用户填写或查看内部数据源名称
...
- 已绑定的数据源上方显示“技能列表：”横条，同一数据源的技能自动换行排列；点击“执行”后全屏打开并自动载入、分析，切换模型后可复用本次数据“重新分析”，首次结果后可通过“继续问”进行携带前文的多轮互动
- 技能测试和执行会自动采集分页及虚拟滚动表格，实时展示页数、滚动次数与累计行数；默认最多 10 页或 1000 行，可停止采集并使用已有数据继续分析，采集后恢复第一页和表格顶部
- 检测到分页数据源时，正式执行会在自动采集前询问载入页数；测试技能先正常打开测试页，用户点击“开始测试”后才询问。可输入 1–10 或“全部”，“全部”最多采集 10 页。自动翻页造成浮层暂时收起时，采集进度及完成事件会恢复对应页面
...
- 点击左侧“技能”可创建技能并在页面上选择数据源；数据源名称由系统自动识别并在内部保存，无需用户填写或查看。刷新后会重新校验并展示“可用、数据源已变化、数据源失效”，也可修改或删除
...
- 技能配置保存在 `storage.local`，按页面 origin + pathname 展示，包含单数据源绑定、自然语言分析方法及页面显示名称；采集结果只保存在当前测试或执行会话内存中。
...
`npm test` 是默认提交门槛，会检查全部 `src/**/*.js` 的语法、Manifest 显式入口、调试日志规范、demo HTML 结构以及纯逻辑单元测试。E2E 会先验证扩展 service worker 已启动，再打开本地 fixture；扩展未加载和内容脚本未初始化会给出不同错误。
