...
- 技能使用单个自然语言输入框描述分析需求，并根据当前内容动态提示可补充的判断依据、输出形式和相关数据源字段
- 配置分析方法后可进入全屏“测试技能”：可视化预览当前已渲染数据的总行数和部分样例，最多提交前 200 行；模型按“结论、分析方法建议、分析过程”流式输出并自动滚动，支持修改后再次测试和保存
- 已绑定的数据源上方显示“技能列表：”横条，同一数据源的技能自动换行排列；点击“执行”后全屏打开并自动载入、分析，首次结果后可通过“继续问”进行携带前文的多轮互动
...
- 点击左侧“技能”可创建技能、填写数据源名称并在页面上选择数据源；刷新后会重新校验并展示“可用、数据源已变化、数据源失效”，也可修改或删除
...
- 技能数据源配置保存在 `storage.local`，按页面 origin + pathname 展示；第一轮只保存单表绑定，不保存或执行分析提示词。
