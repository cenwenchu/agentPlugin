...
- 模型请求只发送必需的 `model`、`messages`，流式请求额外发送 `stream: true`；温度、最大输出等可选生成参数使用模型服务默认值，设置页的输出 Token 仅用于本地上下文预算预留
...
- 技能测试和执行会自动采集普通分页表格，实时展示页数与累计行数；默认最多 10 页或 1000 行，可停止采集并使用已有数据继续分析，能够识别数字页码时会尝试恢复执行前页码
