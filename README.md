...
- 测试和执行都在用户点击开始后检查每个数据源是否分页。可输入 1–30 或“全部”，输入框默认 1；“全部”最多采集 30 页
...
`npm test` 是默认提交门槛，会检查全部 `src/**/*.js` 的语法、Manifest 显式入口、调试日志规范、禁止提交 `.log/.txt` 调试产物、demo HTML 结构以及纯逻辑单元测试。单元测试包括多数据源请求分区、空单元格对齐、模型感知预算、完整性判断、CSV/TSV 解析和表格资源上限。E2E 会先验证扩展 service worker 已启动，再覆盖技能创建、修改、测试、执行、CSV/XLSX 临时上传和刷新持久化。
