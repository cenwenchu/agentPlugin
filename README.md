...
- 技能配置保存在 `storage.local`。Chat 技能页保留全部技能目录和其他页面的快速入口，卡片区展示当前页面相关技能。技能包含 1–5 个独立数据源绑定、自然语言分析方法及数据源显示名称；旧版单数据源技能读取时兼容为 `sources[0]`。测试和执行依次采集全部数据源，全部完成后按独立区块一次性提交模型；采集结果只存在于当前测试或执行会话内存中。
...
`npm test` 是默认提交门槛，会检查全部 `src/**/*.js` 的语法、Manifest 显式入口、调试日志规范、demo HTML 结构以及纯逻辑单元测试，包括多数据源请求分区、空单元格对齐、字符预算公平分配和完整性判断。E2E 会先验证扩展 service worker 已启动，再覆盖技能创建、修改、测试、执行和刷新持久化。
