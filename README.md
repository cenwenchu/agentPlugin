...
- 右键菜单仅保留“打开 AI Chat 浮层”
- 鼠标悬停表格行时显示 checkbox，可逐行或批量选中表格数据加入上下文
- 支持当前可见区域截图和用户拖拽框选的区域截图，并作为视觉上下文发送
...
- 右键菜单：`打开 AI Chat 浮层`
- 打开 Chat 后点击“截图”捕获当前可见区域，或点击“区域截图”拖拽框选；截图可预览、启用/停用或删除
- Chat 顶部可切换模型，下拉框旁通过独立提示标明“支持图片”或“不支持图片”
- 顶部第一行使用最大化、设置图标和“清空”操作，模型切换单独放在第二行
...
- 截图以 JPEG Data URL 保存在页面内存，单次请求最多发送最近 4 张；模型必须支持 OpenAI 兼容的视觉输入。
