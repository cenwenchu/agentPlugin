...
- 按列分析运行期缓存同时保存在页面内存和 `chrome.storage.session`，默认 TTL 2 小时；缓存同时保存结论及 `needsAttention` 关注标记，返回上一页时会优先命中缓存恢复结论与警示样式，不重复请求模型
- 列模式下，`needsAttention: true` 会为整个派生列单元格应用浅红底色；普通完成结果继承原表格底色，加载中和失败状态仍使用独立的状态提示色
- jtv 虚拟滚动会在当前控制器生命周期内按业务行指纹累计派生结果；下拉后再返回已分析区域时直接回填已有结论，不因可见行窗口或批次组合变化而重复请求模型
- jtv 表体使用行变更监听即时补入等宽派生列；虚拟行重建时同步按行指纹查询控制器内存，已分析行直接显示缓存结论，只有新行显示“分析中”，同时保持列宽和表头对齐
- jtv 滚动使旧窗口会话失效时，已经返回的模型结果会先按分析指纹和行指纹落入缓存，再停止旧 DOM 更新；避免快速滚动导致中间批次结果被丢弃而出现结果空洞
- jtv 快速滚动期间会按首次看到的行指纹累计未知记录，并在批次容量内合并后续模型请求；模型漏回、空结论或解析失败的行会优先重试一次，再继续后续行，减少请求碎片和结果顺序空洞
